...
export class TinkerbellRenderer implements Renderer {
  ...
  step(): void {
    ...
    // Tinkerbell typically lives roughly in [-1.4, 0.4] x [-1.7, 0.4].
    const cx = this.width / 2 + this.width * 0.25
    const cy = this.height / 2 + this.height * 0.3
    const scale = Math.min(this.width, this.height) * 0.55
    let x = this.x
    let y = this.y
    for (let i = 0; i < points; i++) {
      ...
      if (!isFinite(x) || !isFinite(y) || Math.abs(x) > 5 || Math.abs(y) > 5) {
        x = -0.72 + (Math.random() - 0.5) * 0.01
        y = -0.64 + (Math.random() - 0.5) * 0.01
        continue
      }
  ...
  draw(): void {
    ...
    for (let i = 0, di = 0; i < len; i++, di += 4) {
      ...
      const rgb = hsl(280 + t * 80, 70, 30 + 50 * t)
      data[di] = rgb[0]
      data[di + 1] = rgb[1]
      data[di + 2] = rgb[2]
      data[di + 3] = 255
...
export function createTinkerbellRenderer(): Renderer {
  return new TinkerbellRenderer()
}
