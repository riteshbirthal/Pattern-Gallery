...
void main(){
  vec2 c = u_center + vec2(v_pos.x * u_aspect, -v_pos.y) / u_zoom; // flip Y so ship sits upright
  vec2 z = vec2(0.0);
  int n = 0;
  float esc = 0.0;
...
export class BurningShipRenderer implements Renderer {
...
export function createBurningShipRenderer(): Renderer {
  return new BurningShipRenderer()
}
